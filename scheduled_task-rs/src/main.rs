...
fn print_scheduled_task(task: &ScheduledTask, indent: usize) {
    ...
    if let Some(display_name) = &p.display_name {
        if !display_name.is_empty() {
            ...
            println!("{pad}|  |-- DisplayName: {}", display_name);
        }
    }
    if let Some(id) = &p.id {
        if !id.is_empty() {
    ...
    if let Some(group_id) = &p.group_id {
        if !group_id.is_empty() {
            println!("{pad}|  |-- GroupId: {}", group_id);
        }
    }
    if let Some(user_id) = &p.user_id {
        if !user_id.is_empty() {
            println!("{pad}|  |-- UserId: {}", user_id);
        }
    }
    if let Some(run_level) = p.run_level {
        println!("{pad}|  |-- RunLevel: {:?}", run_level);
    }
    if let Some(logon_type) = p.logon_type {
        println!("{pad}|  |-- LogonType: {:?}", logon_type);
    }
    for (i, a) in task.actions.iter().enumerate() {
        println!("{pad}|-- Actions:");
        println!("{pad}|  |-- Action {}:", i + 1);
    ...
    for (i, t) in task.triggers.iter().enumerate() {
        ...
        println!("{pad}|  |-- Trigger {}:", i + 1);
        println!("{pad}|     |-- Enabled: {}", t.enabled.unwrap_or(false));

        if let Some(duration) = &t.duration {
            println!("{pad}|     |-- Duration: {}", duration);
        }

        if let Some(interval) = &t.interval {
            println!("{pad}|     |-- Interval: {}", interval);
        }

        if let Some(stop) = t.stop_at_duration_end {
            println!("{pad}|     |-- StopAtDurationEnd: {}", stop);
        }

        println!("{pad}|     |-- Properties:");
    ...
    if let Some(s) = &task.settings {
        ...
        if let Some(enabled) = s.enabled {
            println!("{pad}|  |-- Enabled: {}", enabled);
        }
        if let Some(hidden) = s.hidden {
            println!("{pad}|  |-- Hidden: {}", hidden);
        }
        if let Some(allow) = s.allow_demand_start {
            println!("{pad}|  |-- AllowDemandStart: {}", allow);
        }
        if let Some(allow) = s.allow_hard_terminate {
            println!("{pad}|  |-- AllowHardTerminate: {}", allow);
        }
        if let Some(disallow) = s.disallow_start_if_on_batteries {
            println!("{pad}|  |-- DisallowStartIfOnBatteries: {}", disallow);
        }
        if let Some(stop) = s.stop_if_going_on_batteries {
            println!("{pad}|  |-- StopIfGoingOnBatteries: {}", stop);
        }
        if let Some(limit) = &s.execution_time_limit {
            if !limit.is_empty() {
                println!("{pad}|  |-- ExecutionTimeLimit: {}", limit);
            }
        }
    }
    println!();
